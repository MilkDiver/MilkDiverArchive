...
let canvas,ctx,canvasWidth,canvasHeight,gradient,analyserNode,audioData;
...
const setupCanvas = (canvasElement,analyserNodeRef) => {
	...
	gradient = utils.getLinearGradient(ctx,0,0,0,canvasHeight,[{percent:0,color:"#0033ff"},{percent:.25,color:"#0099ff"},{percent:.5,color:"cyan"},{percent:.75,color:"#33cc99"},{percent:1,color:"#33cc66"}]);
  ...
  // 1 - populate the audioData array with the frequency data from the analyserNode
    ...
    if(params.showWaveform){
        ...
        if (params.showCircles){
            ...
            for(let i=0; i < audioData.length; i++){
                ...
                //yellow-ish circle, smaller
                ctx.save();
                ctx.beginPath();
                ctx.fillStyle = utils.makeColor(200, 200, 111, .5 - percent/5.0);
    ...
    for (let i = 0; i < length; i +=4){
        ...
        if(params.showNoise && Math.random() < .05) {
			...
			data[i] = data[i + 1] = data [i + 2] = 100;// zero out the red and green and blue channels
